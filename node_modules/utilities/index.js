...
function do_rate(username, pdata, next) {
  var tname   = 'products';
  var pid     = pdata.id;
  var rval    = pdata.value; delete(pdata.value); // TEMPORARY
  var one     = 1;
  var updates = [];
  updates.push({operation : "INCREMENT", values : ["rate_count", one]});
  updates.push({operation : "INCREMENT", values : ["rate_total", rval]});
  var params  = {TableName : tname,
                 Key       : {id : pid},
                 Updates   : updates};
  Data.update(params, function(serr, sres) {
    if (serr) next(serr, null);
    else { // UPDATE DISPLATED RATE
      pdata.rate_count += 1;
      pdata.rate_total += rval;
      next(null, null);
    }
  });
}

function do_comment(username, pdata, next) {
  var tname   = 'comments';
  var pid     = pdata.id;
  var comment = pdata.comment; delete(pdata.comment); // TEMPORARY
  var entry   = {username : username, text : comment};
  var updates = [];
  updates.push({operation : "RIGHT_PUSH", values : ["entries", entry]});
  var params  = {TableName : tname,
                 Key       : {productid : pid},
                 Updates   : updates};
  Data.update(params, next);
...
function do_edit_metric(sid, host, ts, is_fav, is_cart, is_rate, is_comment,
                        username, pid, is_add, rval, comment, next) {
  var member = is_fav || is_cart || is_rate || is_comment;
  exports.CheckProduct(pid, function(cerr, pdata) {
    if (cerr) next(cerr, null);
      ...
      if        (is_fav) {
        ...
        action      = do_rate;
        pdata.value = rval; // TEMPORARY: used in do_rate()
      } else if (is_comment) {
        action        = do_comment;
        pdata.comment = comment; // TEMPORARY: used in do_comment()
      } else { // TOP-SELLER
...
exports.EditMemberMetric = function(event, is_fav, is_cart,
                                    is_rate, is_comment, next) {
  ...
  exports.ValidateSessionID(sid, function(verr, username) {
    if (verr) next(verr, null);
      ...
      if (!username) next(new Error(Errors.Message.Authentication), null);
        ...
        var rval    = exports.Number(event, 'value');
        var comment = exports.DecodeUri(event, 'comment');
        do_edit_metric(sid, host, ts, is_fav, is_cart, is_rate, is_comment,
                       username, pid, is_add, rval, comment, next);
      }
    }
  });
...
function handle_redirect(sid, ts, pid, is_fav, is_cart,
                         is_rate, is_comment, next) {
  ...
  exports.EditMemberMetric(event, is_fav, is_cart, is_rate, is_comment, next);
}
...
exports.HandleLogin = function(sid, host, ts, username, redir, pname, next) {
  ...
  var is_rate    = redir  && (redir === 'RATE');
  var is_comment = redir  && (redir === 'COMMENT');
  var is_redir   = is_fav || is_cart || is_rate || is_comment;
  if (!is_redir) {
    ...
    exports.CheckProduct(pid, function(cerr, pdata) {
      if (cerr) next(cerr, null);
        ...
        var pid = pdata.id;
        handle_redirect(sid, host, ts, pid, is_fav, is_cart,
                        is_rate, is_comment, next);
      }
    });
  }
