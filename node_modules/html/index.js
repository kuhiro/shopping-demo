...
var AvK_EastName   = "5ref90y7aa.execute-api.us-east-1.amazonaws.com";
var KuhiroShopName = "shop.kuhiro.com";

function is_pop_domain(host) {
  if (!host) return false;
  var sbd   = host.split(".")[0];
  var ispop = (sbd.split("-")[0].toUpperCase() === "POP");
  return ispop;
}

function get_pop_number(host) {
  var sbd = host.split(".")[0];
  var pn  = sbd.split("-")[1];
  if (isNaN(pn)) return 0;
  else           return Number(pn);
}

function write_switch_pop_select(isc, val, txt) {
    return '<option ' + (isc ? ' selected="selected"' : '') + 
           ' value="' + val + '" > ' + txt + '</option>\n';
}
function write_switch_pop_radio_input(cfunc, isc, val, txt) {
    return '<input type="radio" ' + (isc ? ' checked="checked"' : '') + 
           ' onclick="' + cfunc + '" value="' + val + '" > ' + txt;
}

function write_client_change_domain_js(t) {
  return "client_change_domain(this.value, '" + CustomerName + "', '" +
          t + "');";
}

function create_nine_pop_form(host, isae, isks) {
  var pn     = get_pop_number(host);
  var html   = '';
  var t      = "A2K";
  var cfunc  = write_client_change_domain_js(t);
  var isc    = isks;
  var txt    = "SHOP";
  var val    = "https://" + KuhiroShopName + "/";
  html      += write_switch_pop_radio_input(cfunc, isc, val, txt);
  t          = "K2A";
  cfunc      = write_client_change_domain_js(t);
  isc        = isae;
  txt        = "EC2";
  val        = "https://" + AvK_EastName + "/"; // NOTE API GATEWAY ALWAYS HTTPS
  html      += write_switch_pop_radio_input(cfunc, isc, val, txt);
  html      += "<br/>";
  t          = "A2K";
  cfunc      = write_client_change_domain_js(t);
  html      += 'POP: <select onchange="'+ cfunc + '">\n';
  if (isks || isae) html += '<option>&nbsp;</option>';
  for (var i = 1; i <= 15; i++) {
    var isc  = (pn === i);
    var val  = "https://pop-" + i + ".kuhiro.com/";
    html    += write_switch_pop_select(isc, val, i);
  }
  html      += '</select>\n';
  return html;
}

function get_body_background_color(ispink, isteal) {
  if      (ispink) return '#FFEEEF';
  else if (isteal) return '#ECFEEF';
  else             return '#FFFFFF';
}

exports.Header = function(sid, host, ts) {
  var plink  = sid ? exports.PurchasesLink(sid, ts) : "";
  var isae   = (host === AvK_EastName);
  var isks   = (host === KuhiroShopName);
  var ispop  = is_pop_domain(host);
  var tlink  = "";
  if (isae || isks || ispop) {
    tlink = create_nine_pop_form(host, isae, isks);
  }
  var bgcol   = get_body_background_color(isae, isks);
  var tscript = null;
  if (ts) tscript = '<script>client_record_start_time(' + ts + ');</script>';
  var jsrc    = "https://www.kuhiro.com/static/demo_shop_helper.js";
  var html    = '<html><head><script src="' + jsrc + '"></script></head>' +
...
exports.HomeLink = function(sid) {
  ...
  search     += (search.length ? '&' : '') + ('customer=' + CustomerName);
...
exports.EditCartLink = function(sid, pid, txt) {
  var url     = './edit_asset';
  var sargs   = sid ? '&session=' + sid : '';
  var search  = 'asset=cart&product_id=' + pid + '&add=true' + sargs;
  search     += (search.length ? '&' : '') + ('customer=' + CustomerName);
  return build_anchor(url, search, txt);
}
...
exports.AddFavoriteLink = function(sid, pid, txt) {
  var url     = './edit_asset';
  var sargs   = sid ? '&session=' + sid : '';
  var search  = 'asset=favorite&product_id=' + pid + '&add=true' + sargs;
  search     += (search.length ? '&' : '') + ('customer=' + CustomerName);
  return build_anchor(url, search, txt);
}
...
exports.RateProductLink = function(sid, pid, rval, txt) {
  var url     = './rate_product';
  var sargs   = sid ? '&session=' + sid : '';
  var search  = 'product_id=' + pid + '&value=' + rval + sargs;
  search     += (search.length ? '&' : '') + ('customer=' + CustomerName);
  return build_anchor(url, search, txt);
}

exports.IncludeFavicon = function() {
  return '<link rel="shortcut icon" href="https://www.google.com/favicon.ico" type="image/x-icon">';
}

exports.IncludeAjaxScripts = function() {
  return '<script src="https://www.kuhiro.com/static/demo_shop_helper.js"></script>\n';
}
...
exports.ProductCommentsAjaxDiv = function(sid, username, pid) {
  var url  = './show_product_comments?username=' + username +
             '&product_id=' + pid;
  url     += '&customer=' + CustomerName;
  if (sid) url += '&session=' + sid;
  return add_xml_json_request(url, "comments",
                              DivWidth, DefaultTextHeight, 90, ChildHeight,
                              ChildLeftMargin, true, 4);
}
...
function get_next_load_balanced_host(host) {
  ...
  var dots = host.split(".");
  var sub  = dots[0];
  var nh   = 'https://' + sub + '-' + CurrDomainNum + '.kuhiro.com';
