...
function list_update(uparams, i, dots, val, vname, is_prepend) {
  var kname = "#p" + i;
  initialize_expression_attribute_names(uparams);
  uparams.ExpressionAttributeNames[kname] = dots;
  var op    = is_prepend ? " = list_append(" + kname + ", " + vname + ")" :
                           " = list_append(" + vname + ", " + kname + ")";
  uparams.UpdateExpression += kname + op;
  uparams.ExpressionAttributeValues[vname] = [val];
}
...
function update_dynamo_params_inserts(uparams, inserts, cnt) {
  ...
  for (var i = 0; i < inserts.length; i++) {
    ...
    var vname = ":tv" + cnt;
    if        (pos === 0) {  // LEFT_PUSH
      list_update(uparams, cnt, dots, val, vname, true);
    } else if (pos === -1) { // RIGHT_PUSH
      list_update(uparams, cnt, dots, val, vname, false);
    } else {
...
exports.update = function(params, next) {
  ...
  for (var i = 0; i < updates.length; i++) {
    ...
    } else if (u.operation === "LEFT_PUSH") {
      var dots = u.values[0];
      var pos  = 0;
      var val  = u.values[1];
      inserts.push([dots, pos, val]);
    } else if (u.operation === "RIGHT_PUSH") {
      var dots = u.values[0];
      var pos  = -1;
      var val  = u.values[1];
      inserts.push([dots, pos, val]);
